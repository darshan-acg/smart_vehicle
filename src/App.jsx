...
const percent = (value) => sensorReading(value, "%", 0);
const EMPTY_HARDWARE = { relay1: 0, relay2: 0, battery: null, current: null, voltage: null };
...
// Names a GPS point after the nearest configured drop location, so pickup
// reads as a place instead of bare coordinates. Beyond this radius nothing on
// campus is close enough to name the point.
const NEAREST_NAME_RADIUS_M = 400;
function nearestLocationName(point, dropLocations) {
  if (!point || !Array.isArray(dropLocations)) return "";
  let best = null;
  for (const d of dropLocations) {
    const lat = Number(d.lat);
    const lng = Number(d.lng);
    if (!Number.isFinite(lat) || !Number.isFinite(lng)) continue;
    const away = distanceMeters(point, { lat, lng });
    if (away <= NEAREST_NAME_RADIUS_M && (!best || away < best.away)) best = { away, name: d.name };
  }
  return best?.name || "";
}
...
// Names the ends of a trip that sit outside the boundary the admin drew. A
// trip the vehicle is not allowed to make is not charged for, so payment is
// refused while this returns anything.
function endsOutsideFence(ride, fence) {
  if (!ride) return [];
  return [
    ["Pickup", ride.pickupLat, ride.pickupLng],
    ["Delivery", ride.destinationLat, ride.destinationLng],
  ]
    .filter(([, lat, lng]) => {
      // A blank or missing coordinate is not a point outside the fence, and
      // Number("") would read as 0,0 - an end nobody set is simply skipped.
      if (lat == null || lat === "" || lng == null || lng === "") return false;
      const point = { lat: Number(lat), lng: Number(lng) };
      return isValidLatLng(point) && !isInsideGeoFence(point, fence);
    })
    .map(([label]) => label);
}
...
// Live ESP32 branch, admin only: the three sensorData readings plus the two
// relay lines the dashboard drives. The user dashboard deliberately shows the
// battery percentage alone (see BatteryMetric) - current, voltage and the
// relay states are operator information.
function AdminHardwarePanel({ hardware }) {
...
// The only hardware reading the user dashboard shows.
function BatteryMetric({ hardware }) {
  const hw = hardware || EMPTY_HARDWARE;
  return <Metric icon="■" label="Battery" value={percent(hw.battery)} hint="Live vehicle battery level" />;
}
...
function AdminDashboard({ db, setDb, session, logout, toast, cloudStatus, hardware }) {
  ...
  useEffect(() => {
    const activeItem = mobileNavRef.current?.querySelector("button.active");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav ref={mobileNavRef} aria-label="Admin dashboard sections">
        ...
        <div className="content">
          {tab === "overview" ? (
            <>
              ...
              <AdminHardwarePanel hardware={hardware} />
...
function UserDashboard({ db, setDb, session, logout, toast, dbLoaded, hardware }) {
  ...
  const bookingPickupName = nearestLocationName(bookingPickupPreview, db.dropLocations);
  const bookingEndsOutside = endsOutsideFence({
    pickupLat: bookingPickupPreview?.lat,
    pickupLng: bookingPickupPreview?.lng,
    destinationLat: bookingDeliveryPreview?.lat,
    destinationLng: bookingDeliveryPreview?.lng,
  }, db.settings.geoFence);
  const rideEndsOutside = endsOutsideFence(currentRide, db.settings.geoFence);
  const paymentBlocked = rideEndsOutside.length > 0;
  const outsideEndsText = (ends) => `${ends.join(" and ")} ${ends.length > 1 ? "are" : "is"} outside the permitted boundary`;
  ...
  function bookRide(e) {
    ...
    const ride = {
      ...
      pickupLat,
      pickupLng,
      pickupAddress: bookingForm.pickupAddress.trim() || nearestLocationName({ lat: pickupLat, lng: pickupLng }, db.dropLocations),
  ...
  function openPayment(ride) {
    const outside = endsOutsideFence(ride, db.settings.geoFence);
    if (outside.length) {
      setDb((prev) => appendEvent(prev, {
        type: "PAYMENT_BLOCKED",
        level: "warning",
        actor: user.name,
        message: `Payment refused for ${ride.id}: ${outsideEndsText(outside).toLowerCase()}.`,
      }));
      toast("error", "Outside the geo-fence", `${outsideEndsText(outside)}. No payment is taken for this trip.`);
      return;
    }
    setPaymentRide(ride);
  }
  ...
  function completePayment(method, methodForm) {
    ...
    // The admin can move the boundary while the payment modal is open.
    const outside = endsOutsideFence(paymentRide, db.settings.geoFence);
    if (outside.length) {
      setPaymentRide(null);
      toast("error", "Outside the geo-fence", `${outsideEndsText(outside)}. No payment is taken for this trip.`);
      return;
        ...
        <div className="content">
          {tab === "home" ? (
            <>
              ...
              <div className="metrics-grid five"><BatteryMetric hardware={hardware} /><Metric icon="⌖" label="Geo-Fence" value={insideFence == null ? "GPS Needed" : insideFence ? "Inside" : "Outside"} hint={`${db.settings.geoFence.radiusM} m permitted radius`} /><Metric icon="↗" label="Current Trip" value={currentRide ? currentRide.tripStatus : "No Active Trip"} hint={currentRide?.destinationName || "Book a ride"} /><Metric icon="₹" label="Payment" value={currentRide?.paymentStatus || "—"} hint={currentRide ? money(currentRide.amount) : "No amount due"} /><Metric icon="◎" label="GPS Samples" value={userLocations.length} hint={latestLocation ? `Last: ${formatDate(latestLocation.at)}` : "No location stored"} /></div>
                ...
                <div className="card">
                  ...
                  {currentRide ? (
                    ...
                    <form className="booking-form" onSubmit={bookRide}>
                      ...
                      <div className={`gps-hero ${bookingForm.pickupLat!==""?"live":""}`}><div>⌖</div><b>{bookingForm.pickupLat===""?"Detecting your current location…":bookingPickupName?`Pickup: ${bookingPickupName}`:"Pickup set to your current GPS location"}</b><span>{bookingForm.pickupLat!==""?`${Number(bookingForm.pickupLat).toFixed(6)}, ${Number(bookingForm.pickupLng).toFixed(6)}`:"Allow location access to auto-fill pickup"}</span></div>
                      ...
                      {bookingEndsOutside.length?<div className="geo-alert bad"><b>⚠ {outsideEndsText(bookingEndsOutside)}</b><span>No payment is taken for a trip that starts or ends outside the geo-fence. Pick points inside the boundary.</span></div>:null}
              ...
              {!currentRide ? <div className="card"><Empty text="No active trip. Open Dashboard and book a ride with pickup and delivery coordinates."/><button className="primary center-btn" onClick={()=>setTab("home")}>Book a Ride</button></div> : (
                <>
                  ...
                  <div className="workflow-grid">
                    ...
                    <div className={`workflow-card ${currentRide.paymentStatus==="SUCCESS"?"done":currentRide.otpVerified?"active":""}`}><span className="step">3</span><h3>Dummy Payment</h3><p>Select wallet, net banking, UPI or card. No real payment is made.</p><div className="workflow-amount">{money(currentRide.amount)}</div><button className="primary wide" disabled={!currentRide.otpVerified||currentRide.paymentStatus==="SUCCESS"||paymentBlocked} onClick={()=>openPayment(currentRide)}>{currentRide.paymentStatus==="SUCCESS"?`Paid via ${currentRide.paymentMethod} ✓`:paymentBlocked?"Payment Blocked":"Pay Now"}</button>{paymentBlocked?<div className="pay-blocked">{outsideEndsText(rideEndsOutside)}, so this trip is not charged for.</div>:null}</div>
      ...
      (value) => {
        ...
        setHardware((prev) => {
          ...
          return prev.relay1 === next.relay1 && prev.relay2 === next.relay2 &&
            prev.battery === next.battery && prev.current === next.current &&
            prev.voltage === next.voltage ? prev : next;
        });
        setDbLoaded(true);
    ...
    <>
      ...
      {!dbLoaded ? (
        <div className="cloud-loading">
          <div className="logo big">SV</div>
          <b>Loading the cloud database</b>
          <span>Fetching the latest records from the Firebase Realtime Database…</span>
        </div>
      ) : !session ? (
...
@media(max-width:820px){html{-webkit-text-size-adjust:100%;text-size-adjust:100%;scroll-padding-bottom:84px}.auth-shell{grid-template-columns:1fr}.auth-brand-panel{display:none}.auth-form-panel{padding:17px;min-height:100vh}.auth-card{padding:22px;max-height:none}.sidebar{position:fixed;left:0;right:0;bottom:0;top:auto;width:auto;height:calc(72px + env(safe-area-inset-bottom));padding:7px 8px calc(7px + env(safe-area-inset-bottom));display:block;background:#0d1b33;z-index:50;border-top:1px solid rgba(255,255,255,.1);overflow:hidden}.brand-row,.sidebar-role,.sidebar-bottom{display:none}.sidebar nav{display:flex;flex-wrap:nowrap;align-items:stretch;gap:4px;margin:0;height:58px;overflow-x:auto;overflow-y:hidden;scroll-snap-type:x proximity;scrollbar-width:none;overscroll-behavior-x:contain;-webkit-overflow-scrolling:touch}.sidebar nav::-webkit-scrollbar{display:none}.sidebar nav button{flex:0 0 76px;min-width:76px;padding:5px 4px;display:flex;flex-direction:column;justify-content:center;align-items:center;gap:3px;text-align:center;scroll-snap-align:center}.sidebar nav button i{font-size:17px;line-height:1}.sidebar nav button span{font-size:9px;line-height:1.15;white-space:nowrap}.sidebar nav button.active{box-shadow:none;background:#193255}.main{margin-left:0;padding-bottom:calc(82px + env(safe-area-inset-bottom))}.topbar{height:72px;padding:10px 14px;gap:10px}.topbar>div:first-child{min-width:0}.topbar h1{font-size:17px;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.top-actions{flex:0 0 auto}.date-pill,.user-chip>div{display:none}.content{padding:15px;width:100%;overflow:hidden}.section-header{align-items:flex-start;flex-direction:column}.section-actions{width:100%}.section-actions>*{flex:1}.metrics-grid{grid-template-columns:1fr 1fr;gap:9px}.metric-card{padding:12px}.metric-value{font-size:16px}.metric-icon{width:34px;height:34px}.grid-2-cards{grid-template-columns:1fr}.grid-2{grid-template-columns:1fr}.destination-cards{grid-template-columns:1fr}.workflow-grid{grid-template-columns:1fr 1fr}.payment-methods{grid-template-columns:1fr 1fr}.payment-summary{grid-template-columns:1fr 1fr}.payment-summary .amount{grid-column:1/3}.storage-grid{gap:13px}.storage-stats{grid-template-columns:1fr 1fr}.stack-buttons button{width:100%;min-height:46px}.card code{white-space:pre-wrap;overflow-wrap:anywhere}.welcome-card{padding:18px}.welcome-card p{max-width:70%}.ride-hero{grid-template-columns:45px 1fr}.ride-price{grid-column:2;text-align:left}.map-title-row{align-items:flex-start;gap:8px}.google-map{min-height:320px}}
@media(max-width:520px){.auth-form-panel{padding:0}.auth-card{min-height:100vh;border-radius:0;box-shadow:none;padding:20px 16px}.metrics-grid{grid-template-columns:1fr}.workflow-grid{grid-template-columns:1fr}.sidebar nav button{flex-basis:82px;min-width:82px}.top-actions .badge{display:none}.content{padding:12px}.card{padding:14px;border-radius:14px}.card h3{font-size:16px}.muted,.storage-card p{font-size:13px;overflow-wrap:anywhere}.storage-icon{width:58px;height:58px}.storage-stats span{padding:12px 8px}.stack-buttons button{font-size:14px;padding:12px 10px}.welcome-card{align-items:flex-start}.welcome-status{padding:9px 12px}.welcome-card p{max-width:100%}.destination-row{grid-template-columns:1fr;gap:8px}.button-row{flex-direction:column}.payment-methods{grid-template-columns:1fr 1fr}.modal{padding:16px}.toast{right:12px;bottom:calc(84px + env(safe-area-inset-bottom));width:calc(100vw - 24px)}.section-header h2{font-size:19px}}

@media(hover:none),(pointer:coarse){.destination-cards{grid-template-columns:1fr}.destination-card{grid-template-columns:1fr;padding:14px}.destination-card b{font-size:15px}.destination-card small{font-size:11px}.destination-row{grid-template-columns:1fr;gap:8px}.destination-info b{font-size:14px}.destination-info span{font-size:11px}.destination-actions{justify-content:flex-start}}
`;
